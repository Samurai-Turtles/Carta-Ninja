...
updateWeight dl [idx_to_increase1, idx_to_increase2, idx_to_decrease1, idx_to_decrease2] weight = do
    ...
    let w3 = updateArray w2 in1 value_to_decrase1
    let value_to_decrase2 = (weight !! idx_to_decrease2) + dl
    updateArray w3 in2 value_to_decrase2


updateArray :: [Int] -> idx -> newVal -> [Int]
updateArray arr idx newVal = arr // [(idx, new
Val)]
...
wHandRecursive i cards weights =
    case elem of
        "F" -> replicate (weights !! 0) i ++ wHandRecursive (i + 1) cards weights
        "N" -> replicate (weights !! 1) i ++ wHandRecursive (i + 1) cards weights
        "A" -> replicate (weights !! 2) i ++ wHandRecursive (i + 1) cards weights
        "M" -> replicate (weights !! 3) i ++ wHandRecursive (i + 1) cards weights
        "T" -> replicate (weights !! 4) i ++ wHandRecursive (i + 1) cards weights
